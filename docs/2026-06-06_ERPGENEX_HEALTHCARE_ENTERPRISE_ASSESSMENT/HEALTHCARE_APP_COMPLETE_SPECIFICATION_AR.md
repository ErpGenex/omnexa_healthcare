...
1. موقع `/hospital` → حجز موعد (معالج 5 خطوات، تقدّم تلقائي، AR/EN، إنشاء حساب Portal)  
...
### Wave 4 — مستشفى عالمي + تشغيل (2026.06.16)

| البند | الإصلاح |
|-------|---------|
| بنك الدم | `Healthcare Blood Donor` · `Blood Unit` · `Transfusion Order` + API |
| CSSD | `Cssd Instrument` · `Sterilization Cycle` |
| تعويض الأطباء | `Physician Compensation Rule` · `Settlement` + API |
| جودة وعدوى | `Quality Corrective Action` · `Infection Surveillance Case` + API |
| ديمو عالمي | 39 قسم · 31 طبيب · 50 مريض · 22 خدمة ويب · `world_class_demo_catalog` |
| مساحة عمل | قسم Blood Bank · CSSD · QMS في Healthcare workspace |

### Wave 5 — ICD-11 + إغلاق نهائي (2026.06.16)

| البند | الإصلاح |
|-------|---------|
| ICD-11 | `Healthcare Icd11 Code` + `api/icd11` + crosswalk ICD-10 |
| تشخيص الزيارة | حقل `icd11_code` في `Healthcare Encounter Diagnosis` |
| تقييم المنصة | مجالات `blood_bank` · `cssd` · `physician_compensation` · `quality_management` · `specialty_engine` |
| **`gap_analysis.total_open`** | **0** (جميع الفجوات مُغلقة) |

### Wave 4 (سابق) — إصلاحات تشغيلية (2026.06.12)

| البند | الإصلاح |
|-------|---------|
| حجز Guest — `Healthcare Appointment` | `create_healthcare_appointment(..., ignore_permissions=True)` + تجاوز branch access للضيف |
| الأطباء فارغون بعد اختيار القسم | فلترة `get_published_doctors` عبر Service Catalog + رموز DEMO |
| مساحة عمل Healthcare ناقصة | إعادة بناء القائمة الكاملة (192+ رابط) بعد KPI sync في control-tower |
| اختبارات | `test_guest_book_appointment_online` · `test_published_doctors_by_department` · `test_control_tower_does_not_truncate_sidebar` |
...
bench --site SITE execute omnexa_healthcare.patches.v1_0.sync_healthcare_workspace_full.execute
bench build
...
| test_gap_closure_wave4.py | موجة 2026.06 Wave 4 (Blood Bank · CSSD · QMS) |
| test_gap_closure_wave5.py | موجة 2026.06 Wave 5 (ICD-11 · APIs) |
| test_branch_demo_seed.py | ديمو مستشفى عالمي كامل |
| test_public_hospital_site.py | أقسام/أطباء موقع المستشفى |
| test_healthcare_workspace.py | قائمة مساحة عمل كاملة (192+ رابط) |
