frappe.pages['telemedicine_admin'].on_page_load = function(wrapper) {
    var $wrapper = $(wrapper);
    ...
    var iframeId = 'telemedicine-admin-iframe-' + Date.now();
    ...
    $wrapper.html(`
        <div style="width: 100%; height: calc(100vh - 100px); display: flex; flex-direction: column; background: #f7fafc; position: relative;">
            ...
            <iframe id="${iframeId}" 
                    ...
                    onerror="document.getElementById('${iframeId}-loading').innerHTML='<div style=\\'color: red;\\'>فشل التحميل. <a href=\\'/telemedicine-admin\\' target=\\'_blank\\'>افتح في نافذة جديدة</a></div>'">
            </iframe>
        </div>
    `);
};
