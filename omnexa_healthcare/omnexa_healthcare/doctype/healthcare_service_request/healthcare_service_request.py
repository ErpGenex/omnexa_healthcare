...
class HealthcareServiceRequest(Document):
	...
	def _validate_patient(self):
		...
		if not pdata.active:
			frappe.throw(_("Inactive patient cannot receive service requests."), title=_("Patient"))
	...
	def _validate_encounter(self):
		...
		if row.patient != self.patient:
			frappe.throw(_("Encounter patient must match service request patient."), title=_("Encounter"))
