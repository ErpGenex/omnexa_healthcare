...
MATURITY_DOMAINS: dict[str, dict[str, Any]] = {
	...
	"analytics": {
		...
		"checks": [
			...
			"omnexa_healthcare.api.care_gap_automation",
			"Healthcare Load Test Report",
			"omnexa_healthcare.api.certification_export",
		],
	},
	...
	"interoperability": {
		...
		"checks": [
			...
			"omnexa_healthcare.api.fhir_bulk_export",
			"omnexa_healthcare.api.hl7_messaging",
			"Healthcare X12 Transaction",
			"Healthcare Icd11 Code",
			"omnexa_healthcare.api.icd11",
	...
	"eprescription": {
		...
		"checks": [
			...
			"Healthcare Rxnorm Code",
			"Healthcare Prescription Signature",
			"Healthcare Cds Alert Log",
			"healthcare-erx-writer",
			"healthcare-pharmacy-rx-verify",
			"omnexa_healthcare.api.erx",
		],
	},
	...
	"compliance_security": {
		...
		"checks": [
			...
			"omnexa_healthcare.api.gdpr_erasure",
			"omnexa_healthcare.api.audit_phi",
			"omnexa_healthcare.healthcare_mfa",
			"omnexa_healthcare.compliance_docs",
			"omnexa_healthcare.api.security_rate_limit",
		],
	},
	"specialty_engine": {
		"weight": 4,
...
def run_gap_analysis(maturity: dict) -> dict[str, Any]:
	...
	strategic = [
		...
		{"feature": "Global ePrescription — Medication Request, QR, pharmacy verify", "domain": "eprescription", "priority": "Critical", "status": "completed"},
		{"feature": "RxNorm formulary + smart drug search (1000+ codes)", "domain": "eprescription", "priority": "Critical", "status": "completed"},
		{"feature": "Advanced CDSS — 12 safety checks + override audit", "domain": "clinical_decision_support", "priority": "Critical", "status": "completed"},
		{"feature": "SNOMED subset + LOINC binding audit", "domain": "interoperability", "priority": "High", "status": "completed"},
		{"feature": "GDPR erasure workflow + HIPAA BAA/DPIA templates", "domain": "compliance_security", "priority": "Critical", "status": "completed"},
		{"feature": "PHI audit on 26+ DocTypes + MFA hard block", "domain": "compliance_security", "priority": "Critical", "status": "completed"},
		{"feature": "FHIR MedicationRequest + Bulk $export + HL7 MLLP", "domain": "interoperability", "priority": "High", "status": "completed"},
		{"feature": "Predictive analytics v2 + care gap automation", "domain": "analytics", "priority": "High", "status": "completed"},
		{"feature": "HIMSS EMRAM Stage 7 + JCI digital certification export", "domain": "analytics", "priority": "High", "status": "completed"},
		{"feature": "Consultant sign-off pack + Global #1 evidence", "domain": "analytics", "priority": "High", "status": "completed"},
